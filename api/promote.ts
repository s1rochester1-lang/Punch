...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!employeeId || (role !== "manager" && role !== "employee")) {
    return res.status(400).json({ error: "A valid employeeId and role are required." });
  }

  if (employeeId === session.sub && role !== "manager") {
